...
import { CategoryService } from '../../author/model/category/category.service';
...
export class GameEditComponent implements OnInit {
        ...
        this.categoryService.getCategories().subscribe((categories: any) => {
            ...
            if (this.game.category != null) {
                const categoryFilter: Category[] = categories.filter(
                    (category: any) => category.id == this.data.game.category.id
                );
    ...
    onSave() {
        this.gameService.saveGame(this.game).subscribe((result: any) => {
